...
Local reference clones and downloaded docs should be fetched only after explicit approval. Use `tools/fetch_tier2_references.ps1` for a dry-run preview before fetching. If direct script execution is blocked by local PowerShell policy, run the dry-run with:
