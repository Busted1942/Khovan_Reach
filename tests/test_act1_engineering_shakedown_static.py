...
class Act1EngineeringShakedownStaticTests(unittest.TestCase):
    ...
    def test_engineering_sequence_text_and_objectives_exist_in_order(self) -> None:
        ...
        for phrase in [
            ...
            "Great, everything is checking out so far. Let's test some internal comms systems next.\\nEngineering: Click on one of the crew quarters, click set rally point, and then choose one of your DamCon teams. This will set their idle position. Note that you can read their medical status when you click on DamCon teams. In addition, they will be able to work more efficiently when they are allowed tim in their quarters, mess, gym, and if injured, they should be ordered to report to sickbay. Take care of your crew when the preassure is low and they will take care of you when the preassure is high.",
