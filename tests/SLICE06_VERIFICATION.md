...
**Fixed, not yet re-tested live (2026-08-08):** both code bugs are corrected in `act1_drone_contact_fire.mast`, a trace breadcrumb was added to the previously-silent `Read Target Spike Status` handler, and the destroy-hook guard (cleanup-in-progress flag, consumed before the damage-value fallback) is implemented. `python run_tests.py quick` passes (111 checks, 99 tests) and the MAST compile preflight passes against the installed sbs_utils library. **None of this is live-proven yet** — compile success proves syntax only, not runtime behavior. The next live pass must re-test subsystem detection, the destroy-hook guard's flag branch, and `Read Target Spike Status` (now debuggable via its new trace line) before any of these three can move from "fixed" to "proven."
...
- `Read Target Spike Status` remains live-ambiguous. The handler now has trace logging (`[KHOVAN ACT1 DRONE SPIKE STATUS]`), so the next attempt will produce real evidence, but that attempt has not happened yet.
- Manual subsystem hit detection: the two bugs that caused it to fail are fixed and static-tested, but not yet live re-tested. Do not claim this works until a live pass confirms `drone_target_spike_manual_subsystem_hit_observed` sets correctly on a real hit.
- The destroy-hook guard's `cleanup_in_progress` flag branch is new code, implemented but not yet live-tested. Its damage-value fallback branch is trace-validated from the prior pass.
...
- **SUPERSEDED, then FIXED (not yet live re-tested) — see the trace-verified pass below.** An earlier entry in this section claimed manual subsystem targeting was confirmed live-unavailable and raised it as a Drone 01 design decision. Reading `scripts/acts/act1_drone_contact_fire.mast` against `tests/live_startup_trace.txt` from the same pass showed that was wrong: the trace shows `manual_system=SHPSYS.WEAPONS` fired once (19:12:58), proof Cosmos does expose subsystem-lock info to this hook. The failure was two code bugs, not an API absence, and both are now fixed in the file (still needs live re-test before being trusted):
  ...
  **Status: fixed, compile-clean, static-tested. Not live-proven.** The next live pass must confirm `manual_subsystem_hit_observed` actually sets correctly on a real hit before this is closed out.
...
- **Confirmed live 2026-08-08, root-caused, not yet re-verified after fix:** the operator could kill the target via Weapons but did not know where to read damage on it from any station UI. Likely the same root cause as above. Re-evaluate after a live pass with the fix in place, before assuming a separate UI-visibility problem exists.
- Destruction events fire reliably; the GM-cleanup-vs-real-kill distinction is now implemented as a two-layer guard in `//damage/destroy`: primary signal is a `drone_target_spike_cleanup_in_progress` flag the cleanup handler sets immediately before `sbs.delete_object()` and the destroy handler consumes; fallback signal (used only if the flag is not set) is the confirmed-workable nonzero-damage check. The flag branch is new code and has not yet been live-tested; the damage-value fallback branch is validated against the 2026-08-08 trace (GM cleanup: `0.0`/`0.0`; real kill: climbing nonzero).
- If, after live re-test, subsystem detection still proves unreliable, Drone 01 needs the documented Comms/captain-confirmation fallback as its primary path. That decision point remains deferred, not cancelled.
...
Phase A station mechanics are live-proven with a genuinely 4-station crew (2026-08-08): spawn, non-attack, Science scan (while alive), Comms hail (no stock-menu interference), Weapons selection/fire/kill. Both code bugs are fixed and the destroy-hook guard is implemented (`python run_tests.py quick` passes, 111 checks / 99 tests, MAST compile preflight passes) — **none of this is live-proven yet.** Remaining before Phase A closes:

1. Live pass: retry `Read Target Spike Status` while the target is alive. It now logs `[KHOVAN ACT1 DRONE SPIKE STATUS]` on every attempt, so this pass will finally produce evidence either way.
2. Live pass: fire on the target again and confirm `drone_target_spike_manual_subsystem_hit_observed` sets correctly, now that the AND-gate bug is removed. Also confirm `drone_target_spike_manual_critical_hit_observed` behaves sensibly (it may rarely be true — that's fine, it's tracked independently now).
3. Live pass: confirm the destroy-hook guard's `cleanup_in_progress` flag branch actually fires correctly (GM Cleanup should report `destruction_source=gm_cleanup`; a Weapons kill should report `destruction_source=genuine_weapons_kill`). The damage-value fallback branch is already trace-validated from the prior pass; this specifically tests the new flag-based primary signal.
4. Only if subsystem detection still proves unreliable after the fix — route it to the operator as a Drone 01 design decision. Not before; the prior entry raising this as an immediate decision point was based on an incorrect read of the evidence, corrected above.
...
Stop after Phase A only if the live re-test in items 1-3 shows the fixes did not work as intended, or if subsystem detection is genuinely unreliable even with correct code.
