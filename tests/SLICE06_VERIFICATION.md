...
- **SUPERSEDED, then FIXED (not yet live re-tested) — see the trace-verified pass below.** An earlier entry in this section claimed manual subsystem targeting was confirmed live-unavailable and raised it as a Drone 01 design decision. Reading `scripts/acts/act1_drone_contact_fire.mast` against `tests/live_startup_trace.txt` from the same pass showed that was wrong: the trace shows `manual_system=SHPSYS.WEAPONS` fired once (19:12:58), proof Cosmos does expose subsystem-lock info to this hook. The failure was two code bugs, not an API absence, and both are now fixed in the file (still needs live re-test before being trusted):
  1. ~~Lines 206-207 (pre-fix) read `.get("system_damage", sbs.SHPSYS.WEAPONS)`/`.ENGINES`; the `SHPSYS` enums sat in `.get()`'s unused default-value slot.~~ **THIS ENTRY WAS WRONG AND HAS BEEN REVERSED (2026-08-09.)** `def get(self, name, index=0)` (`sbs_utils/mock/sbs.py:570`) — the second argument is an **index**, never a default, and a missing key returns `None`. The writer indexes by `SHPSYS` (`sbs_utils/procedural/internal_damage.py:305-309`): `for x in range(SBS.SHPSYS.MAX): blob.set('system_damage', cur, x)` over `["weapon", "engine", "sensor", "shield"]`. So `system_damage` **is** per-subsystem — WEAPONS=0, ENGINES=1, SENSORS=2, SHIELDS=3 — and the original code was correct. The 2026-08-08 "fix" made both fields read index 0, so they agreed by construction and looked like one generic total. Original form restored, with None-guards. The error survived a day because the counter-example it cited, `data_set.get("playerThrottle", 0)`, reads index 0 either way.
...
- ~~`system_damage` per-subsystem differentiation is unproven; both fields read one generic value by design.~~ **RESOLVED 2026-08-09** — see the reversal in item 1 above. `system_damage` is indexed by `SHPSYS` and the per-subsystem counts are the number of damaged grid objects carrying that system's role. Cookbook 9.1 carries the settled account; cookbook 7.3 carries the corrected handler.
...
- **DESIGN-VS-ENGINE CONFLICT, raised 2026-08-09, operator decision needed.** `docs/01_design/10_mast_requirements.md` section 8.5 gate 9 requires "Drone 01 Weapons array disabled in three confirmed hits." The engine cannot deliver that on the specified hull. `data/shipData.yaml` gives `kralien_cruiser` `"hullpoints": 2`; the whole Kralien warship range is `kralien_cruiser` 2, `kralien_battleship` 3, `kralien_dreadnought` 4. Manual subsystem targeting only reaches a system once the target's shields are down, so by the time subsystem hits can land the hull is all that remains. Live 2026-08-09: the operator destroyed Drone 01 on the **first** hit. Three hits into a two-point hull is not survivable on any Kralien hull.
...
- **UNPROVEN, raised 2026-08-09 — writing `hull_hit_counter`.** The key is proven as a *read* (`data/missions/common/dmx.py:59`, `blob.get("hull_hit_counter", 0)`, where it is treated as accumulated hull damage), and the paired shield key `shield_val` is proven as a *write* (`legendarymissions/collisions/collision.mast:54-56`). `set_data_set_value()` itself is cookbook-proven [LIVE] (section 9.1). What is unproven is that writing `hull_hit_counter` back to `0` actually prevents destruction — it may be a display counter the engine does not consult for death.
  ...
  Evidence to look for: `[KHOVAN ACT1 DRONE 01 HULL HOLD] restore_count=N` in the trace on each hit, paired with the drone surviving to `[KHOVAN ACT1 DRONE 01 HIT] count=3`. If `HULL HOLD` lines appear but the drone still dies, the key is not the death mechanism. Do **not** escalate by guessing further keys — fall back to `kralien_dreadnought`, then to the GM `Khovan: Fallback Weapons Hit` route, and raise an API-uncertainty finding per cookbook section 12.

  Related but separate: torpedoes. Artemis starts with 10 Homing and 3 Nuke. Either will likely destroy a Kralien cruiser in one event regardless of the hull hold. The drill is a beam exercise; if the operator wants it torpedo-proof, that is a separate change to the Drone 01 setup.

- **Raised and ACTIONED 2026-08-09 on operator direction — the two drones now have opposite combat roles.** Section 8.5 specifies "non-attacking AI" for Drone 01 and 8.6 specifies "combat target = true" for Drone 02; the implementation had drifted from both. Operator direction: "I do not want the first drone to attack, I only want it to be a practice attack. The second should be annoying but not very dangerous."

  - **Drone 01** now spawns on `behav_do_nothing` instead of `behav_npcship`. The `kralien` role is retained deliberately — enemy side is what makes the stock Science route match (`legendarymissions/science_scans/science.mast:144`, `side_are_enemies`) and render the shield-frequency panel, so switching to `neutral` to stop the shooting would trade away the scan the drill is built on. `raider` is only a targeting tag used by autoplay selection helpers, not an AI driver, and is harmless.
  - **Drone 02** was `khovan_training, neutral` on a `tsn_warpster` hull — a neutral contact on a *friendly TSN* hull, which is not a combat target at all: `side_are_enemies()` is false on a neutral side, so it drew neither the stock enemy Science route nor normal hostile Weapons handling. Now `kralien, raider, khovan_drone_02` on `kralien_cruiser` with `behav_npcship`. `kralien_cruiser` is the weakest Kralien warship in `shipData.yaml` (2 beams, 80/80 shields, 2 hull), which fits both 8.6's "lethal pressure = low / training-safe" and the operator's "annoying but not very dangerous". Its 2 hull points are a feature here, since destruction is Drone 02's completion gate — the Drone 01 hull hold is deliberately scoped to `khovan_drone_01` only.

  **UNPROVEN: `behav_do_nothing`.** Not previously used in this repo. Proven usage exists in a shipping reference mission (`HereThereBeMonsters-main/story.mast:304,456,519`) for inert contacts. The live check that matters is whether Drone 01 still renders the stock Kralien Science panel — behavior key is not supposed to affect side or the Science route, but the stock route's `//science` condition includes `has_any_role(SCIENCE_SELECTED_ID, "ship,cockpit")`, and nothing in the Khovan spawn sets those roles explicitly. If the scan panel regresses, revert to `behav_npcship` and raise the non-attacking requirement as a design finding rather than trying other behavior keys blind.
