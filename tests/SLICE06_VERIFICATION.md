...
- **Raised 2026-08-09, not actioned — Drone 01 AI conflicts with design.** Section 8.5 specifies "non-attacking AI" for Drone 01. The current spawn uses roles `kralien, raider` with `behav_npcship`, which is normal hostile AI. The `raider` role was adopted deliberately to match the Spike Target configuration that produces the stock Kralien Science return with shield frequencies, so removing it risks the scan panel fix. Not changed without operator direction; flagged so it is not mistaken for an oversight.
